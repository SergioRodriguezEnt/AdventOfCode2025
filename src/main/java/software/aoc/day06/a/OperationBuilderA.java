...
public class OperationBuilderA implements OperationBuilder {
    private List<String> operands;
    private String operators;
    private int maxLength = 0;

    public OperationBuilderA() {
        operands = new ArrayList<>();
    }

    @Override
    public OperationBuilderA add(String str) {
    ...
    private List<List<Long>> operandLists(List<Integer> indexes) {
        return IntStream.range(0, indexes.size())
                .mapToObj(i -> operands.stream()
                        .map(operand -> Long.parseLong(operand.substring(
                                indexes.get(i),
                                i + 1 < indexes.size() ? indexes.get(i + 1) : maxLength
                        ).strip()
                )
        ).toList()).toList();
    }
